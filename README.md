...
> 실시간 차량 센서 데이터를 클라우드로 수집·처리하고, 이상 탐지 결과를 SQL 인프라와 대시보드로 연동한 End-to-End 파이프라인 프로젝트입니다.
...
* **주요 수행 내용:**
  * **Lambda 아키텍처 구현:** 대규모 스트리밍 부하 분산을 목적으로 실시간 이상 탐지 판단 흐름과 장기 배치 분석 흐름을 물리적 이원화 구조로 설계
  * **수집 및 전처리 파이프라인:** Python 시뮬레이터 센서 데이터를 `IoT Hub`로 안정적 인제스션 후 `Stream Analytics` 기반 실시간 이상 필터링 로직 구현
  * **엔드투엔드 서빙 검증:** 가공 데이터를 `Azure SQL DB`에 최적화 적재 프로세스로 설계하고, `Azure Functions API` 연동으로 웹 대시보드 표출까지의 지연 없는 흐름 구현
